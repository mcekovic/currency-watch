	...
	@Override public CurrencyEvent getEvent(final String currency, final Date date) {
		return Algorithms.find(EVENTS, new Predicate<CurrencyEvent>() {
			@Override public boolean test(CurrencyEvent event) {
				return event.isForCurrency(currency) && Objects.equals(event.getDate(), date);
			}
		});
	}

	@Override public Iterable<CurrencyEvent> getEvents(final String currency, final Date fromDate, final Date toDate) {
		return Algorithms.select(EVENTS, new Predicate<CurrencyEvent>() {
			@Override public boolean test(CurrencyEvent event) {
				return event.isForCurrency(currency) && new DateRange(fromDate, toDate).contains(event.getDate());
			}
		});
	}
}
